...
first. Inputs from `docs/shipyard/<work-slug>/`: `requirements.md`,
...
2. **Build against the spec**, recording as-built deviations in the decision
   log as they occur, not reconstructed at the end.
...
4. **Verify against `requirements.md`, independently of the build path**:
   ...
   report states exceptions against requirements ("REQ-3 unmet because X").
   Test counts and coverage percentages are not verification results and do
   not appear in it.
...
6. **Pause 4: acceptance, framed by blast radius.** Low and reversible:
   state a default with a deadline ("shipping at 2pm unless stopped"). High
   or irreversible: an explicit yes is required; nothing outward-facing or
   irreversible moves on silence or inferred approval. Acceptance transfers
