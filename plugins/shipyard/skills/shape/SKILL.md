...
first. Input: `docs/shipyard/<work-slug>/problem.md` and `decision-log.md`,
...
1. **Define desired state and assess the gap** against the current-state
   baseline.
2. **Generate genuine alternatives.** The shortlist always includes
   "do nothing" and "defer" as scored options; without them the comparison
   is not real.
...
4. **Score options as deltas from the accepted baseline** on cost, risk,
   fit, and effort, crediting controls that already exist. Show score
   drivers, not the full matrix.
5. **Draft the decision memo**: only decisions that belong to the human,
   each pre-filled with a recommendation, plus one pre-mortem prompt
...
7. **Present the pivot, not the math** ("Option B wins unless data residency
   ...
   decisions transfers to the human here. End by naming the next command:
   `/spec`.
