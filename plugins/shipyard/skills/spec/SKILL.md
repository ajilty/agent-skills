...
first. Input: the returned memo at `docs/shipyard/<work-slug>/memo.md` and
...
1. **Derive requirements from the memo decisions**: functional and
   non-functional, every one testable, prioritized (MoSCoW), and traceable
   to the decision it serves.
2. **Produce the build spec**: architecture, design decisions with
   rationale, implementation plan.
3. **Triage every design decision as a one-way or two-way door.**
...
5. **Write the dual outputs** to `docs/shipyard/<work-slug>/`:
   ...
   - append every triaged decision to `decision-log.md`, two-way doors
     included, marked asserted vs confirmed.
