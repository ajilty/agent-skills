...
persona is confined only by the shared session sandbox plus capability subtraction (its
role `sandbox_mode` is **inert** for spawns), so it is **not lane-confined** today;
ADR-0017 designs the fix (per-persona top-level `codex exec --cd`). The runtime hooks need
no per-harness fork; the items below are Codex behaviors, not orchestrate bugs.
...
| 5 | `sandbox_mode` is coarse (`workspace-write`); `writable_roots` is **additive** (adds writable dirs, never subtracts source) and is ignored in `read-only` mode. | **Live limitation.** For the main agent, results-only confinement is carried by `write-scope.sh`, not the sandbox; `writable_roots` cannot narrow a persona to its lane. ADR-0017's per-persona `codex exec --cd <result-dir>` is the OS-level lane confinement (proposed). |
| 6 | **PreToolUse does not fire inside a spawned subagent**, and `spawn_agent` shares the session sandbox (no per-agent knob). | **Live limitation (load-bearing); fix designed in ADR-0017.** The per-persona hook floor is a **main-agent** floor on Codex. A persona spawned in-session is confined only by the shared session sandbox (which in `workspace-write` still allows **source** writes) + capability subtraction — it is **not** lane-confined, and the per-role `sandbox_mode` is **inert** for spawns (an earlier claim here that it confines a spawn was wrong; corrected by live probe). ADR-0017's fix: dispatch each confined persona as its own top-level `codex exec --cd <result-dir>`. Claude Code keeps the per-persona floor natively. |
