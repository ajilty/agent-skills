...
if ! have_opencode; then
  ...
  if [ ! -f "$plug" ]; then
    ...
    grep -q 'env: process.env' "$plug" && pass || fail "opencode install: sh() does not pass env: process.env explicitly — hook subprocesses silently lose RESOLVED_PATH/TOOL_INPUT/PATH on this Bun build (probe-results.md)"

    # subagent.start / session.compacted are the ADR-0034 doc-tension hooks this
    # whole tier exists to bound — test_opencode_compaction_event.sh is the
    # arbiter of whether they're real events, not this static probe. We only
    # assert they're present as declared (informational cross-reference).
    if grep -q '"subagent.start"' "$plug" && grep -q '"session.compacted"' "$plug"; then
      pass   # both UNVERIFIED-in-1.18.4 hooks are wired as documented in the installer's own comments (bounded live by test_opencode_compaction_event.sh)
    else
      fail "opencode install: expected subagent.start + session.compacted hook keys (even if unverified live) — installer output changed shape"
    fi
  fi
