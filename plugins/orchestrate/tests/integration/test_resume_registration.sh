...
if ! have_claude; then
  ...
  cc plugin install orchestrate@ajilty-agent-skills --scope user >/dev/null 2>&1
  ...
  cc2 plugin install orchestrate@ajilty-agent-skills --scope user >/dev/null 2>&1 && pass || fail "stale-settings: install over a stale enabledPlugins entry"
  case "$(cc2 plugin list 2>/dev/null)" in *"orchestrate@ajilty-agent-skills"*) pass;; *) fail "stale-settings: real plugin not listed (stale ghost entry broke it)";; esac
  case "$(cc2 plugin details orchestrate@ajilty-agent-skills 2>/dev/null)" in *"Agents (5)"*) pass;; *) fail "stale-settings: real plugin loads 5 agents despite stale ghost entry";; esac
