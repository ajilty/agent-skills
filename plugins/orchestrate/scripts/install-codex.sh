...
mkdir -p "$AGENTS_DEST" "$DEST/orchestrate-runtime" "$DEST/orchestrate-runtime/personas"
...
# writes are OS-confined to their lane. Appended only to the Codex brain.
cat >> "$BRAIN_DIR/AGENTS.orchestrate.md" <<'BRAINADD'

<!-- Codex dispatch addendum (generated by install-codex.sh; ADR-0017) -->
## Codex: how to dispatch a persona

On Codex, dispatch EVERY persona by running the launcher with the task on stdin, NOT via
`spawn_agent`:

    printf '%s' "<the persona's task>" | \
      orchestrate-runtime/dispatch-persona.sh <persona> <ticket_dir> [<repo_root>]
...
for p in $(yq '.personas | keys | .[]' "$AGENTS"); do
  ...
  body "$bsrc" > "$DEST/orchestrate-runtime/personas/$p.md"
...
Codex install complete ($SCOPE scope).
  ...
  dispatch  -> personas run via orchestrate-runtime/dispatch-persona.sh (codex exec --cd <lane>); NOT spawn_agent (ADR-0017)
